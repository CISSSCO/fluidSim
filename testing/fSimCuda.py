...
module = SourceModule("""
        __device__ double* createZeroMatrix(float x, float y){
            double* arr = ; // create memory on gpu using cudamalloc and then initialize all the elements to zero

        }


        __global__ double* equilibrium(double *rho, double* u, double* v, double* t, double* nx, double* ny){
            double usqr = (3 / 2) * ((u[0] * u[0]) + (u[1] * u[1]));
            double *feq = ; // create memory on gpu using cudamalloc and then initialize all the elements to zero

        }

        __device__ char* obstacle_fun(int *cx, int* cy, int* radius){
            char* function_str = (char*)malloc(100); 
            sprintf(function_str, "(x - %d)^2 + (y - %d)^2 < %d^2", cx, cy, r);
            return function_str;
        }

        __device__ char* inivel(float *uLB, float *ly){
            char* function_str = (char*)malloc(100); 
            const double n = 1e-4;
            const double PI = 3.1415;
            sprintf(function_str, "(1-d) * %.2f * (1+%.4lf*sin(y/%.2f*2*%.2f))", uLB, n, ly, PI);
            return function_str;
        }

        """)
#function to create equation of obstacle
def obstacle_fun(cx, cy, r):
    def inner(x, y):
        return (x-cx)**2+(y-cy)**2<r**2
    return inner

#function to create equation of initial velocity
def inivel( uLB, ly):
    def inner(d,x,y):
